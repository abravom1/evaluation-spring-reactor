...
public class WebExceptionHandler extends AbstractErrorWebExceptionHandler {
    ...
    private Mono<ServerResponse> renderErrorResponse(ServerRequest req) {
        ...
        Map<String, Object> customError = new HashMap<>(); //LinkedHashMap

        //HttpStatus status = HttpStatus.INTERNAL_SERVER_ERROR;
        var status = HttpStatus.INTERNAL_SERVER_ERROR;
        String statusCode = String.valueOf(generalError.get("status"));
        Throwable error = getError(req); //Obtener la exception de la peticion

        //Switch enhanced Disponible desde Java 16/17
        switch (statusCode){
            case "400" -> {
